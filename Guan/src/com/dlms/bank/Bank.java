...
public class Bank {
    ...
    public HashMap<String, Integer> RMMap;
    ...
    public HashMap<String, ArrayList<Customer>> account_HashMap;
    public HashMap<String, Loan> loan_HashMap;
    
    public Bank(String _bankName, HashMap<String, Integer> _port_map, int rm_port, int fe_port, String expectedSequence){
        account_HashMap = new HashMap<String, ArrayList<Customer>>();
        loan_HashMap = new HashMap<String, Loan>();
        
        for(String ch : alphabet){
            account_HashMap.put(ch, new ArrayList<Customer>());
        }
        
        this.bankID = _bankName;
        this.RMMap = _port_map;
        this.rmPort = rm_port;
        this.fePort = fe_port;
    ...
    public void refreshHashMap(){
     ...
     public String openAccount(String bank, String firstName, String lastName, String emailAddress, String phoneNumber, String password) {
        ...
        ArrayList<Customer> list = account_HashMap.get(lastName.toLowerCase().substring(0, 1));
        ...
        if (foundAccount == null) {
            account = new Customer(firstName, lastName, emailAddress, phoneNumber, password, DEFAULT_CREDIT);
            list.add(account);
    ...
    public String getLoan(String bank, String accountNumber, String password, String loanAmount, String sequenceId) {
        ...
        for (ArrayList<Customer> account_list : account_HashMap.values()) {
        ...
        if (foundAccount != null && foundAccount.password.equals(password)) {
            ...
            int count = 0;
            for(String s : RMMap.keySet()){
                if(!s.equals(bankID)){
                    rest_port[count] = RMMap.get(s);
                    count++;
                }
            }
        ...
        synchronized (foundAccount){
            try {
                String content = "transfer" + ":" + loan.ID + "," + loan.accountNumber + "," + loan.dueDate + "," + loan.amount
                        ...
                        + ":";
                BankAsClient client = new BankAsClient("localhost", Integer.valueOf(RMMap.get(otherBank)), content);
                client.run();
                ...
                if (client.result.equals("Yes")) {
                    ...
                    if (loan_HashMap.get(loan.ID) != null) {
                        ...
                        client = new BankAsClient("localhost", Integer.valueOf(RMMap.get(otherBank)), content);
                        client.run();
    ...
    public String printCustomerInfo(String bank) {
        ...
        for (String ch : alphabet) {
            ArrayList<Customer> list = account_HashMap.get(ch);
    ...
    class BankAsReceiver extends Thread{
        ...
        public void run(){
            try {
                ReliableServerSocket serverSocket = new ReliableServerSocket(rmPort);
                ...
                while(!isStopped){
                    ...
                    if(msg.equals("shutdown")){
                        ...
                        if(sequenceId.equals(expectedSequence)){
                            ...
                            switch(requestType){
                                case "openAccount":
                                    result = sequenceId + "%" + rmPort + "#" + openAccount(requestPara[0], requestPara[1], requestPara[2], requestPara[3], requestPara[4], requestPara[5]) + "#";
                                    break;
                                case "getLoan":
                                    result = sequenceId + "%" + rmPort + "#" + getLoan(requestPara[0], requestPara[1], requestPara[2], requestPara[3], sequenceId) + "#";
                                    break;
                                case "transferLoan":
                                    result = sequenceId + "%" + rmPort + "#" + transferLoan(requestPara[0], requestPara[1], requestPara[2]) + "#";

                                    break;
                                case "delayLoan":
                                    result = sequenceId + "%" + rmPort + "#" + delayPayment(requestPara[0], requestPara[1], requestPara[2], requestPara[3]) + "#";
                                    break;
                                case "printCustomerInfo":
                                    result = sequenceId + "%" + rmPort + "#" + printCustomerInfo(requestPara[0]) + "#";
                                    break;
                            }

                            // send result back to front end
                            BankAsClient client = new BankAsClient(feHost,fePort, result);
                            client.start();
                        ...
                        if (request_array[0].equals("search")) {
                            ...
                            ArrayList<Customer> list = account_HashMap.get(content_array[0].toLowerCase().substring(0, 1));
                            Customer foundAccount = null;
                            for (Customer account : list) {
                            ...
                            loan_HashMap.put(loan.ID, loan);
                            List<Customer> list = account_HashMap.get(account.lastName.toLowerCase().substring(0, 1));
                            list.add(account);
                            ...
                            String[] content_array = request_array[1].split(",");
                            Customer foundAccount = null;
                            List<Customer> account_list = account_HashMap.get(content_array[0].toLowerCase().substring(0, 1));
                            for (Customer account : account_list) {
     ...
     private void log(String content){
        ...
        File dir = new File(bankID+""); 
        dir.mkdir();
        
        String path = "./"+ bankID +"/log.txt";
    ...
    private void logCustomer(String id, String content){
        ...
        String path = "./" + bankID + "/Customer"+ id +".txt";
    ...
    private void logManager(String content){
        ...
        String path = "./"+bankID+"/Manager.txt";
