...
//		addInitialPassengerRecord();
	...
	public void initUDP() {
		try {
			aSocket = new DatagramSocket(ServerInfo.getServerMaps().get("NewDelhi"));
			byte[] incomingData = new byte[1024];
			System.out.println("UDP Server is up and listening to " + ServerInfo.getServerMaps().get("NewDelhi"));
			while (running) {
	...
	public boolean editFlightRecord(String recordID, String departure, String destination, int eco,
		...
		synchronized (this) {
			if (index == -1) {// add a new record
				FlightRecord fr = new FlightRecord(recordID, "NewDelhi", destination, eco, busi, fit, dateOfFlight);
...
//		sb.append("NDL" + bookedCount + "\n");
	...
	private void addInitialFlightRecord() {
		flightRecords.add(new FlightRecord(GenerateID.getInstance().getFlightID()+"", "NewDelhi", "Montreal", 100, 100, 100, Config.DEPARTURE_DATE));
		flightRecords.add(new FlightRecord(GenerateID.getInstance().getFlightID()+"", "NewDelhi", "Washington", 100, 100, 100, Config.DEPARTURE_DATE));
//		flightRecords.add(new FlightRecord("0003", "NewDelhi", "Washington", 22, 32, 42, "2016-11-06-10-12"));
//		flightRecords.add(new FlightRecord("0004", "NewDelhi", "Washington", 23, 33, 43, "2016-11-06-10-13"));
//		flightRecords.add(new FlightRecord("0005", "NewDelhi", "Washington", 24, 34, 44, "2016-11-06-10-14"));
//		flightRecords.add(new FlightRecord("0006", "NewDelhi", "Montreal", 20, 30, 40, "2016-11-06-10-10"));
//		flightRecords.add(new FlightRecord("0007", "NewDelhi", "Montreal", 21, 31, 41, "2016-11-06-10-11"));
//		flightRecords.add(new FlightRecord("0008", "NewDelhi", "Montreal", 22, 32, 42, "2016-11-06-10-12"));
//		flightRecords.add(new FlightRecord("0009", "NewDelhi", "Montreal", 23, 33, 43, "2016-11-06-10-13"));
//		flightRecords.add(new FlightRecord("0010", "NewDelhi", "Montreal", 24, 34, 44, "2016-11-06-10-14"));
	...
	private void addInitialPassengerRecord() {
		...
		String[] lastNames = {"Amy", "Ann", "Anjo", "Acor", "Astan"};
		String[] addresses = {"AAA", "BBB", "CCC", "DDD", "EEE"};
		for(int i = 0; i < firstNames.length; i++) {
			Passenger p = new Passenger(firstNames[i], lastNames[i], addresses[i], "2222222");
			PassengerRecord pr = new PassengerRecord(p, "Washington", "fit", "2016-10-01-10-01");
			pr.setPassengerID(100 + i + "");
			aList.add(pr);
		}
		passengerRecordsMap.put('a', aList);
