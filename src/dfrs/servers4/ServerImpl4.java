...
//	public static final String[] SERVERS = new String[] {"Montreal","Washington","New Delhi"};
	...
	public static ServerInterfacePOA getServerImple(int i) {
		return new ServerImpl4(ServerImpl4.SERVER_NAME[i], BaseServerCluster.SERVERS[i],
				ServerImpl4.UDP_PORT_NUM[i], ServerImpl4.T_UDP_PORT_NUM[i]);
	}
	public ServerImpl4(String server, String name, int udp, int tudp) {
		super();
		this.server = server;
		this.name = name;
		UDP_PORT = udp;
...
//	@Override
	public String getBookedFlightCount(String recordType) {
		...
		String value = "";
		if(SERVER_NAME[0].equals(server)) {
			value = server + " " +count+",";
			value +=getCountFromOtherServers(recordType, "localhost", UDP_PORT_NUM[1]);
			value +=",";
			value +=getCountFromOtherServers(recordType, "localhost", UDP_PORT_NUM[2]);
		} else if(SERVER_NAME[1].equals(server)) {
			value =getCountFromOtherServers(recordType, "localhost", UDP_PORT_NUM[0]);
			value += ("," + server + " " +count+",");
			value +=getCountFromOtherServers(recordType, "localhost", UDP_PORT_NUM[2]);
		} else if(SERVER_NAME[2].equals(server)) {
			value +=getCountFromOtherServers(recordType, "localhost", UDP_PORT_NUM[0]);
			value +=",";
			value +=getCountFromOtherServers(recordType, "localhost", UDP_PORT_NUM[1]);
			value += ("," + server + " " +count);
...
//	}
	...
	public String bookFlight(String currentCity, String firstName, String lastName, String address, String phoneNumber,
		...
		Result result = bookFlight(firstName, lastName, address, phoneNumber, destination, flightDate, flightClass);
		if(result.success) {
	...
	public String getBookedFlightCount(String currentCity, String managerID) {
		return getBookedFlightCount(Flight.ALL_CLASS);
	}
...
//		Result result = editFlightRecord(recordID, fieldName, newValue);
//		if(result.success) {
//			return Config.SUCCESS;
//		} else {
			return Config.FAIL;
//		}
	...
	public String transferReservation(String managerID, String PassengerID, String CurrentCity, String OtherCity) {
		Result result = transferReservation(Integer.valueOf(PassengerID), CurrentCity, OtherCity);
		if(result.success) {
			return Config.SUCCESS;
		} else {
			return Config.FAIL;
